<template>
  <div class="container py-5">
    <h2 class="fw-bold mb-4 text-center">팀 정보 수정</h2>
...
<div class="col-lg-3">
  <div class="bg-light rounded p-4 text-center shadow-sm">
    ...
    <img
      :src="previewUrl || (club.logo_path ? `http://localhost:8121${club.logo_path}` : 'https://via.placeholder.com/120')"
      alt="클럽 로고"
          ...
          <div class="row row-cols-1 row-cols-md-2 g-4">
            ...
            <div class="col">
              <div class="border rounded p-4 shadow-sm h-100 bg-white">
                ...
                <div class="mb-1">승: <b>{{ club.win_count }}</b></div>
                <div class="mb-1">무: <b>{{ club.draw_count }}</b></div>
                <div class="mb-1">패: <b>{{ club.loss_count }}</b></div>
                <div class="text-muted mt-2 small">
                  참가수: {{ (club.win_count || 0) + (club.draw_count || 0) + (club.loss_count || 0) }}
                  <span class="ms-3">팀 레벨: <span class="badge bg-primary">{{ teamLevel }}</span></span>
                </div>
              </div>
            </div>
...
<div class="col">
  <div class="border rounded p-3 h-100 bg-light-subtle">
    ...
    <div class="d-flex flex-column gap-2 mt-2">
      <div v-for="member in topMembers" :key="member.user_no"
              ...
              <div class="border rounded p-4 shadow-sm h-100 bg-white">
                ...
                <div class="mb-2">
                  ...
                  <select v-model="clubInfo.age_group" class="form-select">
                    <option value="">선택</option>
                    <option v-for="opt in ageGroupOptions" :key="opt" :value="opt">{{ opt }}</option>
                  </select>
                </div>
                <!-- 활동 요일 -->
                <div class="mb-2">
                  <label class="fw-bold">활동 요일</label><br>
                  <label v-for="day in daysOptions" :key="day" class="me-2 small">
                    <input type="checkbox" :value="day" v-model="clubInfo.active_days" /> {{ day }}
                  </label>
                </div>
                <!-- 활동 시간 -->
                <div class="mb-2">
                  <label class="fw-bold">활동 시간</label><br>
                  <label v-for="time in timeOptions" :key="time" class="me-2 small">
                    <input type="checkbox" :value="time" v-model="clubInfo.active_times" /> {{ time }}
                  </label>
                </div>
              </div>
            </div>
      ...
      <!-- 저장/취소 버튼 (오른쪽 정렬) -->
      <div v-if="club && userNo && club.user_no === userNo" class="text-end mt-4">
        <button @click="submitUpdate" class="btn btn-primary me-2">저장하기</button>
...
onMounted(async () => {
  ...
  try {
    ...
    if (club.value && club.value.club_id) {
      ...
      clubInfo.value = {
        ...clubInfoRes.data,
        active_days: clubInfoRes.data.active_days ? clubInfoRes.data.active_days.split(',') : [],
        active_times: clubInfoRes.data.active_times ? clubInfoRes.data.active_times.split(',') : []
      }
...
watchEffect(() => {
  console.log('userNo:', userNo && userNo.value, 'type:', typeof (userNo && userNo.value))
  console.log('club.user_no:', club.value && club.value.user_no, 'type:', typeof (club.value && club.value.user_no))
  console.log('같은가?:', club.value && userNo && Number(club.value.user_no) === Number(userNo.value))
})
</script>
