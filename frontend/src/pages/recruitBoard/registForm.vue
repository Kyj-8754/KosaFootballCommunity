<template>
  <div class="container my-5" style="max-width: 900px;">
    ...
    <form @submit.prevent="submitForm">
      ...
      <!-- ✅ 클럽 선택 드롭다운 (팀장이 여러 팀을 가질 수 있으므로) -->
      <div class="mb-3">
        <label class="form-label">소속 클럽</label>
        <select v-model="selectedClubId" class="form-select" required>
          <!-- ❗ 드롭다운에서 선택 안 했을 경우를 방지하기 위한 기본 option -->
          <option disabled value="">소속 클럽 선택</option>

          <!-- ✅ 서버에서 받아온 클럽 목록을 반복 렌더링 -->
          <option v-for="club in myClubs" :key="club.club_id" :value="club.club_id">
            {{ club.club_name }}
          </option>
        </select>
      </div>
...
const submitForm = async () => {
  // ✅ 클럽 선택 여부 확인
  if (!selectedClubId.value || isNaN(selectedClubId.value)) {
    alert('⚠️ 소속 클럽을 선택해주세요.')
    return
  }
  ...
  try {
    ...
    await axios.post(`/recruits_api/regist`, {
      ...
      club_id: selectedClubId.value        // 드롭다운에서 선택된 클럽 ID
    }, {
      headers: {
